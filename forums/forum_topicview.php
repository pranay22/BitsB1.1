...
if ( ! defined( 'IN_TBDEV_FORUM' ) )
{
    ...
    $res = @mysql_query("SELECT p. * , u.username, u.class, u.avatar, u.av_w, u.av_h, u.signature, u.sig_w, u.sig_h,
...
//..rp..
  ...
  //			$avatar = "{$TBDEV['pic_base_url']}disabled_avatar.gif";
        ...
        if ($CURUSER["signatures"] == "yes")
        {
         $signature = $arr['signature'] ? "</font><p style='vertical-align:bottom'><br /><font class='statusbartext'>____________________________________________________________</font><br /><img width='{$arr['sig_w']}' height='{$arr['sig_h']}' src='".htmlentities($arr['signature'], ENT_QUOTES)."' alt='' /></p>" : "";
        }
