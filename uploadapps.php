...
if (!$action || $action == "show") {
    ...
    if ($count == 0) {
        ...
        $where1 ="";
        $res = mysql_query("SELECT uploadapp.*, users.id AS uid, users.username, users.class, users.added, users.uploaded, users.downloaded FROM uploadapp INNER JOIN users on uploadapp.userid = users.id $where1") or sqlerr(__FILE__, __LINE__);
...
if ($action == "viewapp") {
    ...
    </table><div align='center'><a href='{$TBDEV['baseurl']}/uploadapps.php'>Return to uploader applications page</a></div>";
    }
...
if ($action == "acceptapp") {
    ...
    $msg = sqlesc("Congratulations, your uploader application has been accepted! You have been promoted to Uploader class and you are now able to upload torrents. Please make sure you have read the [url={$TBDEV['baseurl']}/rules.php]guidelines on uploading[/url] before you do.\n\nNote: $note");
    ...
    mysql_query("UPDATE users SET class = 4, modcomment = " . sqlesc($modcomment) . " WHERE id=$arr[uid]") or sqlerr(__FILE__, __LINE__);
    ...
    $subres = mysql_query("SELECT id FROM users WHERE class = 6,7,8,9") or sqlerr(__FILE__, __LINE__);
