...
loggedinorreturn();
parked(); // uncomment out if you use this :P
...
if ((isset($_GET["do_it"])) || (isset($_POST["do_it"]))) {
    ...
    <table width='650'>
    <tr><td class='colhead3' colspan='2'>
    &nbsp; Report: " . str_replace("_" , " ", $type) . "</td></tr>" . "
    <tr><td class='clearalt6' colspan='2' align='center'>
    <img src='{$TBDEV['pic_base_url']}warned.gif' alt='warned' title='Warned' border='0' /> {$lang['report_report']} <b>" . str_replace("_" , " ", $type) . "</b> {$lang['report_id']} <b>$id</b>" . "
    <img src='{$TBDEV['pic_base_url']}warned.gif' alt='warned' title='Warned' border='0' /><br />{$lang['report_report1']} <a class='altlink' href='rules.php' target='_blank'>{$lang['report_rules']}</a>?</td></tr>" . "
