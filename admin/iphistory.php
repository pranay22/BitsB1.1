...
require_once "include/geoipcity.inc"; 
require_once "include/geoipregionvars.php"; 
 
/*  Custom function which has been posted here but you probably don't have this: 
function write_logs($text,$type) { 
  $text = sqlesc($text); 
  $type = sqlesc($type); 
  $added = time(); 
  mysql_query("INSERT INTO userlog (added, txt, type) VALUES($added, $text, $type)") or sqlerr(__FILE__, __LINE__); 
  } 
*/ 
    ...
    if (get_user_class() < UC_MODERATOR) 
      stderr("{$lang['stderr_error']}", "{$lang['stderr_denied']}"); 
   ...
   $HTMLOUT = ''; 
    ...
    $HTMLOUT .= "<table border='1' cellspacing='0' cellpadding='5'> 
        ...
        { 
                ...
                mysql_query("UPDATE ips SET seedbox=1 WHERE id = $ipid") or sqlerr(); 
                } 
...
if ($banarr[0] == 0) 
if ($ipcount > 1){ 
$ipshow = "<b><a class='altlink' href='ipsearch.php?ip=". $iphistory['ip'] ."'><font color='black'>" . $iphistory['ip'] ."</font></a></b>"; 
} else { 
$ipshow = "<a class='altlink' href='ipsearch.php?ip=". $iphistory['ip'] ."'><b><font color='blue'>" . $iphistory['ip'] ."</font></b></a>"; 
} 
else 
$ipshow = "<a class='altlink' href='/testip.php?ip=" . $iphistory['ip'] . "'><font color='red'><b>" . $iphistory['ip'] . "</b></font></a>"; 
        ...
        // end fetch geoip code 
                ...
                if ($seedbox == '0') { 
                $seedbox = "<a href='admin.php?action=iphistory&amp;id=$id&amp;setseedbox=$iphistory[id]'><font color='red'><b>{$lang['iphistory_no']}</b></font></a>"; 
    $HTMLOUT .= "<tr> 
                ...
                <td class='heading2' align='center'><a href='admin.php?action=iphistory&amp;id=$id&amp;remove=$ipid&amp;deleteip=$userip&amp;username2=$username'><b>{$lang['iphistory_delete']}</b></a></td> 
                <td class='heading2' align='center'><a href='admin.php?action=bans&amp;banthisuser=$username&amp;banthisip=$userip'><b>{$lang['iphistory_ban']}</b></a></td> 
                </tr>"; 
                } else { 
                $seedbox = "<a class='altlink' href='admin.php?action=iphistory&amp;id=$id&amp;setseedbox2=$iphistory[id]'><font color='Green'><b>{$lang['iphistory_yes']}</b></font></a>"; 
                $color = '#CCFFFF'; 
    $HTMLOUT .= "<tr> 
                ...
                <td class='heading2' align='center' style='background-color:$color'><a href='admin.php?action=iphistory&amp;id=$id&amp;remove=$ipid&amp;deleteip=$userip&amp;username2=$username'><b>{$lang['iphistory_delete']}</b></a></td> 
                <td class='heading2' align='center' style='background-color:$color'><a href='admin.php?action=bans&amp;banthisuser=$username&amp;banthisip=$userip'><b>{$lang['iphistory_ban']}</b></a></td> 
                </tr>"; 
                }  // End Seedbox Check 
    } 
    $HTMLOUT .= "</table>"; 
 
    print stdhead("{$username}'s IP History") . $HTMLOUT . stdfoot();  
?>