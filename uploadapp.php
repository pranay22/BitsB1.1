...
if (isset($_POST["form"]) != "1") {
    ...
    elseif ($arr['status'] == 'rejected')
	   ...
	   <input name='scene' type='radio' value='no' checked='checked' />{$lang['uploadapp_no']}</td>
        ...
        <input name='form' type='hidden' value='1' />
                ...
                <p align='center'><input type='submit' name='Submit' value='{$lang['uploadapp_send']}' /></p></td>
        </tr>
        </table></form>
        </td></tr></table>";
    }
    ...
    if (!$res) {
        ...
        $dt = sqlesc(time());
        $subres = mysql_query('SELECT id FROM users WHERE class = 6') or sqlerr(__FILE__, __LINE__);
