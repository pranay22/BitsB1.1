...
function dltable($name, $arr, $torrent)
{
    ...
    foreach ($arr as $e) {
      ...
      $htmlout .= "<td align='left'>" . htmlspecialchars(getagent($e["agent"], $e['peer_id'])) . ((get_user_class() >= UC_ADMINISTRATOR) ? "<a href='ban_client.php?agent=".$e["agent"]."&peer_id=".bin2hex(substr($e["peer_id"], 0, 8 ))."&returnto=".urlencode("details.php?id=".intval($_GET["id"]))."'><img src='pic/smilies/thumbsdown.gif' border='0' alt='Ban client?'></a>" : "")."</td>\n";
      $htmlout .= "</tr>\n";
    }
    $htmlout .= "</table>\n";
    return $htmlout;
    ...
    if(!empty($banned_clients))
        $HTMLOUT .= "&nbsp;<h1>{$lang['peerslist_banned']}<a href='client_clearban.php?returnto=".urlencode("peerlist.php?id=".$row["id"])."'><b>{$lang['peerslist_click']}</b></a></h1>";
 }
